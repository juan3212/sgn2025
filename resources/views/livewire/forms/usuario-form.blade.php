<div class="flex items-center justify-center min-h-screen bg-gray-100">
    <div class="w-full max-w-md p-8 space-y-6 bg-white rounded shadow-md">
        ...
        <form wire:submit.prevent="submit" class="space-y-4">
            ...
            @if($usuarioId)
                <div>
                    <label for="estado" class="block text-sm font-medium text-gray-700">Cambiar Estado</label>
                    <select name="estado" id="estado" required wire:model="selectedEstado" class="w-full px-3 py-2 mt-1 border rounded shadow-sm focus:outline-none focus:ring focus:border-blue-300">
                        <option value=true @if(!$estado) selected  @endif>Matriculado</option>
                        <option value=false @if($estado) selected  @endif>Retirado</option>
                    </select>
                </div>
            @endif
            <div>
