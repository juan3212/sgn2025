    ...
    public function getStudents(Request $request)
    {
        ...
        $students = Usuario::leftJoin("usuario_grado", function ($join) {
            ...
            ->leftJoin("notas", function ($join) {
                $join
                    ->on("usuarios.id", "=", "notas.estudiante_id")
                    ->where("notas.actividad_id", "=", $this->actividad_id);
            })
            ->join("model_has_roles", function ($join) {
                $join->on("usuarios.id", "=", "model_has_roles.model_id");
            })
            ...
            ->select(
                ...
                "usuarios.nuip",
                "notas.valor",
            ) // Selecciona solo los campos de la tabla usuarios
    ...
    public function table(Request $request)
    {
        ...
        return DataTables()
            ...
            ->addColumn("rates", function ($student) {
                return '<span contenteditable="true" class="editable-cell" data-id="' .
                    $student->id .
                    '">' .
                    $student->valor .
                    "</span>";
        ...
        try {
            ...
            return response()->json(
                [
                    "success" => false,
                    "message" =>
                        "Error al guardar las notas en la base de datos",
                    "error_type" => "database",
